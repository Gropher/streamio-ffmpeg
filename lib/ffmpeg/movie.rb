  ...
  class Movie
    ...
    def initialize(path)
      ...
      if video_stream
        ...
        @video_codec, @colorspace, resolution, video_bitrate = video_stream.scan(commas_except_in_parenthesis).map(&:first).map(&:strip)
    ...
    def width
      (rotated? ? resolution.split("x")[1].to_i : resolution.split("x")[0].to_i) rescue nil
    end

    def height
      (rotated? ? resolution.split("x")[0].to_i : resolution.split("x")[1].to_i) rescue nil
    end

    def calculated_aspect_ratio
      r = aspect_from_dar || aspect_from_dimensions
      r = 1/r if rotated?
      return r
    end

    def rotated?
      [90, -90, 270, -270].include?(@rotation)
    end
    ...
    def cropdetect(limit=24, round=8, reset=0)
      # ffmpeg will output to stderr
      command = "#{FFMPEG.ffmpeg_binary} -i #{Shellwords.escape(path)} -vf \"select='isnan(prev_selected_t)+gte(t-prev_selected_t,3)',cropdetect=#{limit}:#{round}:#{reset}\" -f null /dev/null"
      crops = ""
      Open3.popen3(command) do |stdin, stdout, stderr, wait_thr|
        ...
        begin
          ...
          stderr.each_line do |line|
            fix_encoding(line)
            result = line.scan(/crop=([0-9]+\:[0-9]+\:[0-9]+\:[0-9]+)/)
            crops = result.last[0].to_s if result.length > 0
          end

        rescue Timeout::Error => e
          FFMPEG.logger.error "Process hung...\n@command\n#{@command}\n"
          raise Error, "Process hung."
        end
      ...
      crops = crops.split(":")
      return nil if crops.length < 4
      return {:width => crops[0].to_i, :height => crops[1].to_i, :cropx => crops[2].to_i, :cropy => crops[3].to_i}
